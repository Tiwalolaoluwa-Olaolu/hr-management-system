...
// Leave request status enum (per swagger LeaveRequestStatus): 0 = Pending, 1 = Approved, 2 = Rejected
const APPROVED = 1;
...
const EmployeesOnLeave = () => {
  ...
  useEffect(() => {
    ...
    // NOTE: There is currently no list endpoint for leave requests in the API
    // (the spec only exposes POST /api/LeaveRequests and GET /api/LeaveRequests/{id}).
    // This call is left in place — plugged straight into the "on leave" column below —
    // so that once a list endpoint (e.g. GET /api/LeaveRequests) ships on the backend,
    // this page lights up with no further wiring. Until then it fails gracefully and
    // surfaces the error via the Toast below, while the employee list/search still works.
